...
import gallery from '../../images/car-wash.gif'
import axios from "axios";
import { addBlogs,addNotifications,addServices,addQuestions, updateBlogs, updateServices } from "../../../../redux/astroSlice";
...
console.log("your props",props)
    ...
    try {
      ...
      if (!isUpdate) {
        ...
        const response = await axios.put(
          `https://apigarage.njssolutions.in/api/v1/service/update/${id}`,
          formDataToSubmit
        );

        console.log("send the data to server for updation",formDataToSubmit)
        dispatch(updateServices(response.data.data))
      }
  ...
  return (
    <div>
      <div className="row">
        <h4 className="text-center">{isUpdate?"Update Service":"Create New Service"}</h4>
